    ...
    class PresupuestoService:
        ...
        def obtener_detalles_bauart_por_proyecto(db, proyecto_id):
            try:
                ...
                presupuesto = ModelCompras.get_presupuesto_por_proyecto(db, proyecto_id)
                if not presupuesto:
                ...
                tiene_proveedor_cero = ModelCompras.verificar_proveedor_en_detalles(db, presupuesto.id_presupuesto)
                ...
                detalles_presupuesto = ModelCompras.get_detalles_by_presupuesto(db, presupuesto.id_presupuesto)
                resultados = []

                for detalle in detalles_presupuesto:
                    if detalle.id_proveedor == 0:
                        # Obtener el presupuesto Bauart del detalle
                        bauart = ModelCompras.get_bauart_by_detalle(db, detalle.id_detalle)
                        if bauart:
                            # Paso 4: Obtener los detalles Bauart asociados al presupuesto Bauart
                            detalles_bauart = ModelCompras.get_detalles_bauart_by_presupuesto(db, bauart.id_presupuesto_bauart)
                            resultados.append({
