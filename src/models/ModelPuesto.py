...
class ModelPuesto():
    ...
    def alta_puesto(cls,db,puesto):
        try:
            ...
            with db.cursor() as cursor:
                query = """
                    INSERT INTO PUESTO (PUESTO, SUELDO_BASE, SUELDO_TARJETA,
    ...
    def get_all_puestos_no_block(cls, db):
        try:
            with db.cursor() as cursor:
                query = "SELECT * FROM PUESTO WHERE IS_BLOCKED = 0"
                cursor.execute(query)
                rows = cursor.fetchall()
                puestos = []
                for row in rows:
    ...
    def get_puestos_by_id(cls, db,id):
        try:
            with db.cursor() as cursor:
                query = "SELECT * FROM PUESTO WHERE ID = ?"
    ...
    def change_status(cls, db, id, is_blocked):
        try:
            with db.cursor() as cursor:
                query = "UPDATE PUESTO SET IS_BLOCKED = ? WHERE ID = ?"
    ...
    def filter_puesto(cls, db, puesto, estado):
        try:
            with db.cursor() as cursor:
                cursor = db.cursor()
                query = "SELECT * FROM PUESTO WHERE 1=1"
                params = []
