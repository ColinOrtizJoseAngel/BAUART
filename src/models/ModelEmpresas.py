            ...
            with db.cursor() as cursor:
                query = """
                    INSERT INTO EMPRESAS (
                        ...
                        ESTADO, CIUDAD, DIRECCION, FECHA_REGISTRO, USUARIO_ID, IS_BLOCKED
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?);
                """
                cursor.execute(query, (
                    ...
                    empresa.fecha_registro, empresa.usuario, empresa.is_blocked
                ))
                db.commit()
            ...
            with db.cursor() as cursor:
                ...
                for row in rows:
                    empresas.append(Empresas(
                        ...
                        estado=row[18], ciudad=row[19], direccion=row[20], fecha_registro=row[21], usuario=row[22], is_blocked=row[23]
                    ))
                return empresas
        except Exception as ex:
            raise Exception(ex)
        ...
        try:
            with db.cursor() as cursor:
                ...
                cursor.execute(query, (
                    ...
                    empresa.estado, empresa.ciudad, empresa.direccion, empresa.fecha_registro, empresa.usuario, empresa.is_blocked, empresa.id
    ...
    def filter_empresas(cls, db, repse, razon_social, rfc, cp, estado):
        try:
            with db.cursor() as cursor:
                ...
                for row in rows:
                    empresas.append(Empresas(
                        id=row[0], razon_social=row[1], rfc=row[2],  repse=row[3], regimen_fiscal_id=row[8],
                        nombre_representante1=row[4], apellido_representante1=row[5], telefono_representante1=row[6], correo_representante1=row[7],
                        nombre_representante2=row[9], apellido_representante2=row[10], telefono_representante2=row[11], correo_representante2=row[12],
                        nombre_apoderado=row[13], apellido_apoderado=row[14], telefono_apoderado=row[15], correo_apoderado=row[16],cp=row[17],
                        estado=row[18], ciudad=row[19], direccion=row[20], fecha_registro=row[21], usuario=row[22], is_blocked=row[23]
                    ))
